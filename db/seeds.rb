...
project_1 = Project.create!(title: "Colorado Trail Restoration", date: 2.weeks.from_now, organizer: user_2, description: "The Colorado Trail has a 0.2 mile section which has been heavily eroded and is in desperate need of renovation. Come help us rebuild the base and seed nearby ground cover to help prevent future erosion.", active: true, image: 'https://picsum.photos/id/1018/400/300')
...
project_5 = Project.create!(title: "Dome Rock Loop Trail Extension", date: 15.days.from_now, organizer: user_2, description: "We will be adding an extension loop to the Dome Rock Loop Trail, which will reach out to a new overlook. From here, we will have fantastic views of both Dome Rock and Pike's Peak, and typical weather should permit seasonal access from very early spring through late fall. This project will span multiple weeks, so please check the date of this copy before signing up.", active: true, image: 'https://picsum.photos/id/136/400/300')
...
project_6 = Project.create!(title: "National Trails Day Celebration!", date: "2020-06-01", organizer: user_2, description: "Come out and enjoy outdoor trails in celebration of National Trails Day! No volunteer work is organized for this event, so please use this 'project' as a collection place to organize carpools to your favorite trails.", active: true, image: 'https://picsum.photos/id/17/400/300')
address_p6_1 = Address.create!(owner: project_6, line_1: "Trailhead Parking Lot", line_2: "Four Mile Rd", city: "Divide", state: "CO", zip: "80814", longitude: '-106.0384', latitude: '39.4817')
...
puts "--SEEDING CARPOOLS--"
drivers = demo_users.sample(12, random: rng) + [user_1, user_2, user_3, user_4]
drivers.each do |driver|
  vehicle = driver.default_vehicle || Vehicle.create!(owner: driver, make: "Honda", model: "Civic", color: "Silver", year: 2012, fuel_efficiency: 35, fuel_type: "Gasoline", fuel_efficiency_unit: "MPG", passenger_limit: rng.rand(2..5), default: true)
...
puts "and #{CarpoolPassenger.count} passengers"
puts "in #{end_time - start_time} seconds."
